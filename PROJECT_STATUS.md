...
- Current slice: Group 4 endpoint configuration, signing-secret protection, and owner HTTP lifecycle completed.
...
- Backend baseline: Java 25, Spring Boot 4.1.0, Maven, Spring Web MVC, Spring JDBC/Hikari, Hibernate/JPA, Flyway, PostgreSQL, Spring Security with JDBC Spring Session, Lombok 1.18.46 as a compile-time processor, Testcontainers 2.0.5, ArchUnit 1.4.2, and a required strict runtime-mode contract. V2 creates `owner_accounts`, V3 adds technical Spring Session tables, V4 creates `projects`, V5 creates `project_api_keys`, and V6 creates `webhook_endpoints` plus `endpoint_subscriptions`. Identity supports opt-in bootstrap and generic credential verification; API mode has completed browser authentication and owner-scoped project, API-key, and endpoint REST access.
...
- Physical tables or entity mappings beyond `owner_accounts`, `projects`, `project_api_keys`, `webhook_endpoints`, `endpoint_subscriptions`, and technical Spring Session tables; lock SQL remains unimplemented.
- Publisher event and delivery HTTP API; worker delivery behavior; worker runtime contains no delivery behavior yet.
...
Begin publisher event acceptance and atomic initial routing in the `delivery` module: authenticate a publisher API key, require idempotency, persist the event, and create the matching delivery snapshot set in one short PostgreSQL transaction. Keep worker claim, outbound HTTP, retries, and replay out of that slice.
