...
Five cross-module read capabilities are deliberately allowed inside `delivery` transactions:
...
5. retry finalization and `UNKNOWN` recovery read one row-locked, bounded endpoint retry-policy snapshot before persisting the next due-time.
...
Group 6 uses `READ COMMITTED`, `FOR UPDATE SKIP LOCKED` for bounded due delivery candidates, a final endpoint `FOR UPDATE` recheck, and partial pending/claimed claim indexes. Group 7 uses a `READ COMMITTED` transaction that row-locks the current claimed delivery, row-locks the endpoint configuration through its public snapshot contract, conditionally verifies that no `STARTED` attempt exists at the mutation boundary, writes `STARTED`, and extends the lease. V9 independently enforces one `STARTED` attempt per delivery. The snapshot carries URL plus opaque encrypted signing material; only the later outbound adapter may request its transient plaintext. Group 14 uses an application-owned `TransactionTemplate`: it selects an eligible event, locks all graph deliveries with `SKIP LOCKED`, then locks the event, rechecks eligibility, and removes the complete graph in dependency order. This matches replay's source-delivery-then-event-FK lock order; worker scheduling never owns the transaction. No delivery code imports an endpoint repository.
